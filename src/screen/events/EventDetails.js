...
export default function EventDetails(props) {
  // console.log(props?.route?.params?.eventId,">>>>>>Dsad")
  ...
  useEffect(() => {
    ...
    dispatch(tigerFriendListingRequest(obj));
    if (
      props?.route?.params?.eventId &&
      props?.route?.params?.eventId !== undefined
    ) {
      dispatch(EventDetailsRequest({id: props?.route?.params?.eventId}));
    }
  }, [isFocus]);
  ...
  const friendItem = ({item}) => {
    ...
    console.log(item?.friend_details, '>>>>>>>>resss');
    return (
      <>
        {AuthReducer?.getProfileRes?.data?._id != item?._id && (
          <View style={styles.friend_container}>
            ...
            <TouchableOpacity
              style={
                // shareList.includes(item?._id)
                //   ? styles.send_button_outline
                //   :
                styles.send_button_active
              }
              onPress={() => {
                // if (!shareList?.includes(item?._id)) {
                dispatch(
                  sendRequestRequest({
                    to_user_id: item?.friend_details?._id,
                ...
                // setShareList(prev => [...prev, item?._id]);
              }}
              // }}
              // disabled={shareList.includes(item?._id)}
            >
              <Text
                style={
                  // shareList.includes(item?._id)
    ...
    <SafeAreaView
        ...
        <View
          ...
          <View
            ...
            <TouchableOpacity
              ...
              onPress={() => setIsUserSearchModal(true)}>
              <Image
                source={Icons.share}
      ...
      <Modal
        ...
        <View
          ...
          <View style={{flex: 1}}>
            <FlatList
              data={FriendReducer?.friendListingRes.data}
          ...
          <View
            ...
            <TouchableOpacity
              ...
              onPress={() => setIsUserSearchModal(false)}>
              <Text
                style={{
                  color: Colors.white,
                  fontFamily: Fonts.OpenSans_SemiBold,
